        ...
        if client.user.mentioned_in(message):
            ...
            if leaderboard.get(user_id_str, {}).get('solved', False):
                ...
                if message_content == correct_flag:
                    ...
                    thread_name = set  # You need to determine the actual thread name
...
async def on_ready():
    ...
    if forum_channel and guild:
        ...
        try:
            ...
            embed.add_field(name="Flag Submission", value=f"```⚠️BE SURE TO @ THE BOT FOLLOWING YOUR FLAG SUBMISSION. Ex: @PhreakBot CTF{flag_text}```", inline=True)
